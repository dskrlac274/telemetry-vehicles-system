...
public class PosluziteljKazni {
	...
	public final Pattern predlozakKazna = Pattern.compile("^VOZILO (?<id>\\d+) (?<vrijemePocetak>\\d+) "
			+ "(?<vrijemeKraj>\\d+) (?<brzina>-?\\d+(\\.\\d+)?) (?<gpsSirina>-?\\d+\\.\\d+) (?<gpsDuzina>-?\\d+\\.\\d+) "
	...
	public final Pattern predlozakStatistika = Pattern.compile("^STATISTIKA (?<vrijemeOd>\\d+) (?<vrijemeDo>\\d+)$");
	 ...
	 *                                 formatu
	 */
	public void preuzmiPostavke(String datotekaKonfiguracije) throws NeispravnaKonfiguracija, NumberFormatException {
	 ...
	 * kazne, detalja kazne ili statistike.
	 *
	...
	public String obradaKazna() {
		...
		System.out.println("Id: " + kazna.id() + " Vrijeme od: " + jdu.format(kazna.vrijemePocetak()) + " Vrijeme do: "
				+ jdu.format(kazna.vrijemeKraj()) + " Brzina: " + kazna.brzina() + " GPS: " + kazna.gpsSirina() + ", "
				+ kazna.gpsDuzina());
	...
	public String obradaDetaljiKazne() {
		...
		if (najnovijaKazna != null) {
			...
			komanda.append("OK ").append(najnovijaKazna.vrijemeKraj()).append(" ").append(najnovijaKazna.brzina())
					.append(" ").append(najnovijaKazna.gpsSirinaRadar()).append(" ")
	...
	public PodaciKazne dohvatiPodatkeKazne() {
		...
		for (PodaciKazne kazna : sveKazne) {
			if (kazna.id() == idVozila && kazna.vrijemePocetak() >= vrijemeOd && kazna.vrijemeKraj() <= vrijemeDo) {
				if (najnovijaKazna == null || kazna.vrijemePocetak() > najnovijaKazna.vrijemePocetak()) {
