...
import edu.unizg.foi.nwtis.dskrlac20.vjezba_04_dz_1.posluzitelji.radnici.RadnikZaRadare;
...
public class PosluziteljRadara {
	...
	private final Pattern predlozakRegistracije = Pattern.compile("^(?<datotekaKonf>[\\w.]+)$");
	private final Pattern predlozakBrisanja = Pattern.compile("^(?<datotekaKonf>[\\w.]+) OBRIŠI (?<id>\\d+)$");
	private final Pattern predlozakBrisanjaSve = Pattern.compile("^(?<datotekaKonf>[\\w.]+) OBRIŠI SVE$");
	...
	public void pokreniPosluzitelja() {
		try (ServerSocket mreznaUticnicaPosluzitelja = new ServerSocket(podaciRadara.mreznaVrataRadara())) {
			while (!Thread.currentThread().isInterrupted()) {
				var mreznaUticnica = mreznaUticnicaPosluzitelja.accept();
				tvornicaVirtualnihDretvi.newThread(new RadnikZaRadare(this, mreznaUticnica)).start();
			}
		} catch (IOException e) {
			System.out.println(e.getMessage());
		}
	 ...
	 */
	private boolean registrirajSe() {
		StringBuilder komanda = new StringBuilder();
		komanda.append("RADAR ").append(podaciRadara.id()).append(" ").append(podaciRadara.adresaRadara()).append(" ")
				.append(podaciRadara.mreznaVrataRadara()).append(" ").append(podaciRadara.gpsSirina()).append(" ")
				.append(podaciRadara.gpsDuzina()).append(" ").append(podaciRadara.maksUdaljenost());

		String odgovor = MrezneOperacije.posaljiZahtjevPosluzitelju(podaciRadara.adresaRegistracije(),
				podaciRadara.mreznaVrataRegistracije(), komanda.toString());

		return odgovor != null && odgovor.matches("^OK.*");
	}
