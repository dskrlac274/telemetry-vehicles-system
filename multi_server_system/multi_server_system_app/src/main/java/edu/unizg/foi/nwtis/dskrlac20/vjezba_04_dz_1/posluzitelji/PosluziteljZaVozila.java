...
import edu.unizg.foi.nwtis.dskrlac20.vjezba_04_dz_1.posluzitelji.radnici.RadnikZaVozila;
...
public class PosluziteljZaVozila implements Runnable {
	 ...
	 */
	public PosluziteljZaVozila(CentralniSustav centralniSustav) {
	...
	@Override
	public void run() {
		try (AsynchronousServerSocketChannel serverskiKanal = AsynchronousServerSocketChannel.open()) {
			...
			while (!Thread.currentThread().isInterrupted()) {
				...
				if (aktivniKanali.size() < centralniSustav.maksVozila) {
					...
					tvornicaVirtualnihDretvi.newThread(new RadnikZaVozila(this, klijentskiKanal)).start();
				}
			}
