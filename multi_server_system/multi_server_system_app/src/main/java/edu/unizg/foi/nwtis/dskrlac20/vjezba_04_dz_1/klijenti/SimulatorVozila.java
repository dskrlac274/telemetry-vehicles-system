package edu.unizg.foi.nwtis.dskrlac20.vjezba_04_dz_1.klijenti;
...
public class SimulatorVozila {
	...
	private final Pattern predlozakPokretanja = Pattern
			.compile("^(?<datotekaKonf>\\S+\\.(txt|xml|bin|json)) (?<datotekaSimulacije>\\S+\\.csv) (?<id>\\d+)$");
	...
	public static void main(String[] args) {
		...
		} catch (NeispravnaKonfiguracija | IllegalArgumentException | IOException | InterruptedException
	 ...
	 *                                 formatu
	 */
	private void preuzmiPostavke(String datotekaKonfiguracije) throws NeispravnaKonfiguracija, NumberFormatException {
	 ...
	 */
	private void otvoriKlijentskiKanal() throws IOException, ExecutionException, InterruptedException {
	 ...
	 */
	private void posaljiKomanduVozilu(String[] dijelovi) throws InterruptedException, IllegalArgumentException {
		...
		komanda.append("VOZILO ").append(idVozila).append(" ").append(redniBroj).append(" ")
				.append(String.join(" ", dijelovi));
		klijentskiKanal.write(ByteBuffer.wrap(komanda.toString().getBytes(StandardCharsets.UTF_8)));
	}
